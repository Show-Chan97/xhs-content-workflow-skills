...
一组可独立安装的 Codex Skills，用于小红书趋势采集、每日监控、批量竞品研究、七段用户决策路径拆解、笔记表现诊断、飞书多维表格日数据复盘，以及内容电商品牌会议的执行拆解。
...
| [`xhs-competitor-research`](xhs-competitor-research/README.md) | 直接分析附件或分批采集公开笔记，完成证据化竞品研究 |
...
| [`xhs-decision-path-mining`](xhs-decision-path-mining/SKILL.md) | 从批量同品类笔记中拆解七段用户决策路径，生成可追溯的内容元素库 |

七个 Skill 互相独立，可以只安装其中一个，也可以全部安装。
...
仓库内 Skill 只要涉及飞书，读取、写入、链接解析、搜索、群消息、任务创建和回读验证都必须通过 `lark-cli` 执行。不会改用浏览器界面、MCP/App 连接器、通用飞书能力或直接 OpenAPI。
...
三个采集 Skill 优先使用 Codex 自带的 Chrome 控制能力。需要在主执行器发生非策略性技术故障时使用备用方案，可另外安装 `web-access`：

```bash
npx skills add eze-is/web-access
```

`web-access` 是可选外部依赖，不包含在本仓库，也不会被 `install.sh` 或 `install.ps1` 自动安装。它当前要求 Node.js 22+，并需按自身 `SKILL.md` 完成 Chrome 或 Edge 的浏览器准备。三个采集 Skill 会在需要切换时按名称动态发现并完整读取当前安装版本，不写死 Windows 盘符、macOS 用户目录或其他机器路径。

备用执行器只处理 Codex Chrome 插件缺失、控制连接失败或执行器运行异常等技术故障。切换前必须在执行确认单中预先授权，或在故障发生后另行确认。登录墙、验证码、平台风控、网络/站点故障，以及任何明确禁止替代路径的安全或访问策略拒绝，都不会触发备用切换。

无论使用哪个浏览器执行器，关键词、时间范围、互动门槛、详情页预算、只读边界、串行低频和 `note_id` 去重规则保持不变；任何飞书操作仍只通过 `lark-cli` 执行。
...
./install.sh xhs-decision-path-mining
```
...
.\install.ps1 xhs-decision-path-mining
```
...
```text
使用 $xhs-decision-path-mining，分析这批同品类小红书笔记，按七段用户决策路径生成可追溯的内容元素库。
```
...
- 三个采集类 Skill 仅在主浏览执行器发生符合条件的技术故障且已获授权时切换 `web-access`，不得用备用执行器绕过明确的安全、网络、组织、域名或平台策略拒绝；
- 决策路径拆解只分析用户已提供的样本，不自行启动采集，也不把高频元素写成已验证转化规律；
...
- 仓库内 Skill 的所有飞书操作统一通过 `lark-cli` 执行，不使用浏览器、连接器或直接 API 作为备用路径；
...
七个 Skill 均已通过 `quick_validate.py` 结构校验。仓库还会在 macOS、Windows PowerShell 5.1 和 PowerShell 7 中自动试装全部 7 个 Skill，并检查所有文本为 UTF-8 无 BOM；其中六个涉及飞书的 Skill 还会检查飞书 CLI 规范一致且每个 Skill 都明确引用该规范。三个采集类 Skill 均包含首次使用引导、推荐设置、客户配置卡、执行确认门，以及 Codex Chrome 主执行器与 `web-access` 技术备用执行器的切换契约。`xhs-competitor-research` 的附件审计已覆盖完整、缺字段、小样本、重复、冲突、嵌套 JSON 和 XLSX 用例。`xhs-trend-content` 已完成信息不完整和无依据功效表达两组前向测试；`xhs-note-performance-diagnosis` 已完成规则脚本测试、边界回归和独立诊断场景前向测试；`xhs-base-daily-ops` 已完成标准三表、结构歧义、未确认写入和缺少数据源四组独立场景测试；`content-commerce-meeting-execution` 已完成正常输入、缺失信息和空泛会议三组前向测试，并通过同一 Base 内十四字段行动与动态复盘表和完整报告文档的交付契约检查。`xhs-decision-path-mining` 已通过结构、编码和安装烟测；真实业务数据前向测试仍待完成。真实 Chrome 到 `web-access` 的切换仍需在客户环境中验证。
