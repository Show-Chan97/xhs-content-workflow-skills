...
需要结构化跟进时，可以继续要求创建会议复盘 Base 交付包。Skill 会先预览核心问题、板块、解决方案、优先级、负责人、协同人、截止日期、状态和备注九字段行动表，以及将写入 Base 内文档的完整五段式报告；只有明确回复“确认执行”后，才会通过 `lark-cli` 新建独立 Base、行动表和报告文档，并分别回读核验。
