...
description: 围绕指定品类和关键词采集近期高互动小红书公开笔记，提炼趋势、评论反馈和表达结构，再根据已确认的品牌、产品与事实资料，按固定日报骨架生成完整趋势报告与多角度内容草稿。首次使用或信息不完整时，以面向客户的分轮交互完成设置并生成可复用配置卡。用于品牌日更、代运营选题、产品种草草稿、近期趋势采集；不用于长期每日监控或大样本竞品统计研究。
---
...
- 默认把使用者视为不了解采集参数的业务客户，不让客户填写技术配置表。
- 客户只说“开始设置”或仅调用本 Skill 时，必须进入首次引导，不得直接罗列全部必填字段。
- 客户说“按推荐设置”时，自动填充非阻塞默认项，只追问品牌、产品、事实依据、目标人群和交付位置等不能代填的内容。
- 面向客户显示“设置中、等待确认、执行中、需要客户操作、已完成”等自然语言，不单独抛出内部状态码。
- 新任务无法保证自动继承旧设置；用【客户配置卡】提供可复制的复用方式，不声称已经永久保存。
